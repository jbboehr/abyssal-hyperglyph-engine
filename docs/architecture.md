...
A PHPUnit 12.5.33 workload exposes a more fundamental class-linking limitation. A sequential PHPStan creator and a second sequential invocation with the same load path can reuse the generation, but PHPStan's normal parent/worker load paths can fail in `zend_try_early_bind()` or `zend_do_inheritance_ex()` while reading restored class metadata. This remains reproducible after matching the PHP binary, ASLR-disabled personality, loaded modules, Zend extensions, OPcache settings, `sys_temp_dir`, and ini scan path; it also remains after a complete sequential warm-up. Disabling OPcache's inheritance-cache callbacks alone does not make the remaining class-name/map-pointer state safe. Fixed virtual addresses are therefore necessary but not sufficient for unrelated CLI processes. The real-project benchmark intentionally stops before collecting timings until this is resolved.

The failure reduces to one cached user class extending the internal `IteratorIterator` class. The creator links and instantiates it, while a fresh attached CLI process can jump into the `class_IteratorIterator_methods` data table when instantiating the same persisted class. Identical source may pass or fail when only its absolute path changes, confirming that the retained linked class contains creator-process internal-class or function metadata whose accidental validity depends on allocation layout. [`scripts/reproduce-class-linking.sh`](../scripts/reproduce-class-linking.sh) installs the minimal files at a known failing private path and runs the two processes.

PHP core already has a `ZEND_OPCACHE_SHM_REATTACHMENT` build path for this problem. Among its safeguards, an internal parent disqualifies a class from OPcache's inheritance cache; PHP currently defines the capability only on Windows. The next experiment should enable those core safeguards for the AHE PHP build, not merely the separately built OPcache module, and run the minimal reproducer before retrying PHPUnit. A fork-based PHP supervisor remains the fallback if the portable reattachment path still leaves process-local class state.
